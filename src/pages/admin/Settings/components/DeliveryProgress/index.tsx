...
import { DeliveryProgressModal } from "./modals/DeliveryProgressModal";
import { useGetDeliveryProgress } from "@/queries/admin/useGetAdminSettings";
import { Spinner } from "@/components/Spinner";
import { FiEdit } from "react-icons/fi";
import { deleteDeliveryProgress } from "@/services/adminSettings";
...
const DeliveryProgress = () => {
  ...
  const { data, isLoading, isSuccess, isError } = useGetDeliveryProgress({
    page: currentPage,
  });
  useEffect(() => {
  ...
  const { mutate: deleteService, isPending: pendingDelete } = useMutation({
    mutationFn: (id: string) => deleteDeliveryProgress(id), // ✅ call with correct shape
  ...
  return (
    <div>
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[700px] w-full relative">
              <div className="flex justify-between text-left px-3 xl:px-4 py-4 text-md font-inter font-semibold bg-purple300 w-full">
                <span className="flex-1">Delivery Progress</span>
      ...
      <DeleteModal
        ...
        open={openDelete}
        title={"Delete delivery progress"}
        data={info?.name ?? ""}
        id={info?.id ?? ""}
